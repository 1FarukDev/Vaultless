...
function cleanContent(content, findings) {
    ...
    for (const finding of findings) {
        ...
        const varNameMatch = line.match(/(?:const|let|var)?\s*([A-Z_a-z][A-Z_a-z0-9]*)\s*=/);
        if (varNameMatch) {
            const varName = varNameMatch[1].toUpperCase();
            // replace the hardcoded value with process.env reference
            lines[lineIndex] = line.replace(/=\s*["'][^"']+["']/, `= process.env.${varName}`);
        }
    }
    return lines.join("\n");
}
